...
db creation script (cf habitat doc)
...
Otherwise, if you wish to restore the database only partially, you will have to specify which files you want to restore : e.g. `pg_restore -U postgres -f h_med_central.pgc `.
...
To use data from the psql DB straight from R you can use the [DBI](https://cran.r-project.org/web/packages/DBI/DBI.pdf) package. I will briefly explain how to use the main features of the package but more details are available in the doc (@DBI). 
