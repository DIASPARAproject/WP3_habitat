...
fstock_lvl <- dbGetQuery(con_diaspara, 
  "SELECT * FROM refbast.tr_area_are
  WHERE are_id = ANY(ARRAY[1,2]) OR are_are_id = ANY(ARRAY[2,21,22])
  ORDER BY are_id;")
...
## WIP
Here the are_are_id for subdivision is wrong. it should be 504 and 505. To be fixed later on.
:::
...
This next layer is optional, it is added here as an example to show what it could look like. Only catchments flowing into the Baltic Sea are selected for each countries.

A function is created here to insert the country layer into the talbe. This `insert_country_baltic` function takes a country name as input and inserts a new row into the `refbast.tr_area_are` table representing the specified country at the `'Country'` level. It does so by selecting and merging (`ST_Union`) all catchments from the `tempo.catchments_baltic` table that intersect with the country’s geometry in `ref.tr_country_cou`. The resulting area is non-marine (`are_ismarine = false`), and the function automatically assigns a unique `are_id` from the sequence, while setting the `are_are_id` to `3` to indicate that it is nested under the inland stock level (@fig-inlandstocklvl).
...
:::{.callout-note appearance="simple"}
## WIP
Some overlaps are happening at borders. Decisions will be taken later on to decide on how to deal with this.
:::
...
fstock_lvl <- dbGetQuery(con_diaspara, 
  "SELECT * FROM refbast.tr_area_are
  WHERE are_id = ANY(ARRAY[1,3]) OR are_are_id = 3
  ORDER BY are_id;")
...
:::{.callout-note appearance="simple"}
## WIP
An overlap is happening in Estonia, it will be fixed later on.
:::
...
fstock_lvl <- dbGetQuery(con_diaspara, 
  "SELECT * FROM refbast.tr_area_are
  WHERE are_id = ANY(ARRAY[1,3]) OR are_are_id = ANY(ARRAY[3,13])
  ORDER BY are_id;")
...
![Map of WGBAST first assessment units level.](images/fig-riverlevel.png "A view of Scandinavia with polygons surrounding each countries that has a catchment flowing into the Baltic"){#fig-riverlvl}
