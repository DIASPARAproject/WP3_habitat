...
description: "Design an international database of habitat of migratory fish, version = final"
...
To create the habitat database we decided to use a single river network. This ensures a consistent 
network across countries, with the similar spatial resolution and attributes. We decided to use the Hydrological data and maps based on SHuttle Elevation Derivatives at multiple Scales (**HydroSHEDS**).
...
## From a hydroshed spatial layers to a diadromous habitat db {#sec-hcatch}
...
One of the objective of the habitat database is to allow to link any habitat (e.g. any river reaches, lake etc.) with any spatial/bioligcal units used by expert groups when analysing the population dynamics and assessing the species (e.g. stock unit, eel management unit, ecoregion, countries, sea basin). In this context, the main task was to build this association between HydroSHEDS spatial features of continental habitats with spatial units used by WGBAST, WGEEL and WGNAS.
Once the data are downloaded we first split them into smaller groups following ICES Areas and Ecoregions.
...
![Map of all catchments present in the database.](images/fig-all_catchments.png "A view of all European and Northern African catchments "){#fig-all_catchments}
...
### Step 4 : Intersecting the most downstream point with ICES areas

#### Baltic

ICES divisions are already used in the Baltic by ICES WGBAST thus we have kept this same structure. It is divided in three areas grouping together ICES divisions 30 & 31 ; 27,28,29 & 32 and 22,24,25 & 26.
...
The most downstream points are gathered from `tempo.riveratlas_mds`, and separated into different ecoregions using spatial filters based on both countries and ICES ecoregions.

First, we retrieve distinct downstream points that are within 0.04 degrees of ecoregions described by the the geometries in `ices_ecoregions.ices_ecoregions_20171207_erase_esri`, specifically selecting `objectid = 11`, which corresponded to the desired ICES ecoregion. Additionally, these points have to be within 0.02 degrees of the geometries in `tempo.ne_10m_admin_0_countries`, ensuring they are located in Norway or Sweden (in this case, when working on the northern part of the North Sea).  

Similarly, we selected downstream points that are within 0.04 degrees of the ICES areas as described by geometries in `ices_areas.ices_areas_20160601_cut_dense_3857`, specifically arround the sound (subdivision 23), the area separating the Baltic and the Kattegat, while also ensuring they are within 0.02 degrees of Norway or Sweden. This is done because of an overlap between ICES ecoregions and ICES areas in this particular zone. 
...
To create the database, we use database schema (think of it like a folder structure) to separate the dataset. Based on the previous groups based on ecoregions, the data is stored in the different schema. We first illustrate this process using one of the baltic areas as an example.

A first new schema `h_baltic_3229_27` is created to store the selected river segments. Within this schema, we create the `riversegments` table by selecting distinct river segments from `tempo.hydro_riversegments_europe` that match the `main_riv` values found in `tempo.ices_areas_3229_27`. This ensures that only river segments associated with the relevant ICES subdivisions are included.  
...
### Step 7 : Retrieve all missing endorheic catchments using a bounding hull

The previous process was repeated on the whole area. However, one of the problem encountered when working with the most downstream reach of each river is that some rivers do not drain to the sea.
These Endorheic streams terminate inland in lakes, or salt flaps or disappear in underground rivers in karstic environments.
They are more numerous in the Mediterranean. When trying to identify all the points attached to a coastline, we end up with rivers to attached to the final dataset.  

To solve the problem we use a table named `tempo.oneendo_bisciber` generated from a concave hull around the exterior boundary of the merged catchment shapes from `h_biscay_iberian.catchments`. This process creates a generalized polygon representing the area covered by these catchments. To improve spatial query performance, we added a **GIST** index on the geometry column.  
...
![Map of catchments on the Iberian peninsula with missing endorheic basins.](images/fig-endo_biscay.png "A view of the Iberian peninsula with missing endorheic basins"){#fig-endo_biscay}
...
![Map of catchments on the Iberian peninsula without missing endorheic basins.](images/fig-fendo_biscay.png "A view of the Iberian peninsula without missing endorheic basins"){#fig-fendo_biscay}
...
![Map of catchments on the Baltic with missing islands basins.](images/fig-missing_baltic2732.png "A view of the Baltic with missing islands"){#fig-missing_baltic2732}

![Map of catchments on the Baltic without missing islands basins.](images/fig-full_baltic2732.png "A view of the Baltic without missing islands"){#fig-full_baltic2732}
...
Once the hydronetwork is built and the link with ICES areas/ecoregions defined, the next step is to make the link with working group spatial units.
...
Several levels have been layed out (@fig-area_hierarchy_bast).
...
We insert a new spatial unit into the `refbast.tr_area_are` table to represent a marine stock area labelled **"Baltic marine"**. This unit is associated with the **"Stock"** level (`are_lev_code`) and is marked as marine (`are_ismarine = true`). 
...
Two subdivision groupings are in use in the WGBAST database to store some historical data. They are dividing the Baltic into two main areas and are structured as followed : Gulf of Bothnia 300 (`27.3.d.30`,`27.3.d.31`) and Main Baltic 200 (`27.3.d.22`,`27.3.d.23`,`27.3.d.24`,`27.3.d.25`,`27.3.d.26`,`27.3.d.27`,`27.3.d.28`,`27.3.d.29`).
<!--Hilaire : generally the d is not displayed in the notation, but I guess you keep it for consistency with ICES vocab. Cédric Yes it's consistent with table content-->
...
This next layer is used to report national data, but it cannot be a full part of the hierarchy as catchments cross country boundaries.
...
In the hierarchy, the assessment unit hierarchical level is similar to the country (both have inland stock as a parent), but the hierarchical unit progress below assessment unit while there is no lower level below country (@fig-area_hierarchy_bast)
...
#### River
Within each assessment unit, rivers separated depending on their outlet. This is used to select specific rivers within each assessment unit with ease.
...
::: {.callout-caution}
Some names for the smallest rivers are missing. Further improvement in the Vocabulary will be done by ICES.
:::

#### River section

In the WGBAST dataset, some rivers are further split into river parts, the split is done according to the presence of hatchery produced salmon in some sections of the rivers, or according to the presence of impassable dam.
...
The work done on the structural hierarchy for WGNAS follows what has been done for WGBAST, although there are some differences in the layers used. Futhermore, in parallel of the Assessment Unit (stock unit) level, Fisheries level, and post-smolt areas have been added (@fig-area_hierarchy_nas).
```{dot}
...
#### Post smolt areas
After some disscussions with WGNAS, it was decided to use postsmolt areas from [Olmos et al. (2021)](https://doi.org/10.1111/gcb.14913). Those areas where recreated as close as possible using ICES Divisions.

This query inserts a new spatial unit into the `refnas.tr_area_are` table, representing a **marine post smolt area** called **"postsmolt 1"**. It begins with a Common Table Expression (`WITH geomunion`) that selects and merges (`ST_Union`) the geometries (`geom`) from the `ref.tr_fishingarea_fia` table. The filter targets rows where the area level (`fia_level`) is `'Division'` and the division code (`fia_division`) matches one of the values in the array: `'21.4.X'`, `'21.4.W'`, or `'21.5.Y'`.

The resulting unified geometry is inserted as a single polygon into the `geom_polygon` column. A new area ID (`are_id`) is automatically generated using `nextval('refnas.seq')`, the parent area is set to `2` (`are_are_id = 2`), the area code is labelled `'postsmolt 1'`, and the level is defined as `'Post_smolt_area'` via `are_lev_code`. The area is explicitly marked as marine (`are_ismarine = true`), and no line geometry is provided (`geom_line = NULL`).

```{sql}
#| eval: FALSE
#| echo: TRUE
#| code-summary: Code to create the post smolt area 1 level for WGNAS
#| warning: FALSE
#| message: FALSE
...
SELECT nextval('refnas.seq') AS are_id,
       ...
       'Post_smolt_area' AS are_lev_code,
        true AS are_ismarine,
        geom AS geom_polygon,
		NULL AS geom_line
        FROM geomunion;
...
```{r}
#| label: refnas.tr_area_are marine Post smolt area level
#| echo: FALSE
#| warning: FALSE
#| message: FALSE
#| tbl-cap: Structure of the marine post smolt area in the referential table


fstock_lvl <- dbGetQuery(con_diaspara, 
  "SELECT are_id, are_are_id, are_code, are_lev_code, are_wkg_code, are_ismarine FROM refnas.tr_area_are
  WHERE are_id = ANY(ARRAY[1,2,4]) OR are_lev_code = 'Post_smolt_area'
...
![Map of WGNAS post smolt area level.](images/fig-postsmolt.png "A view of of the North Atlantic with polygons surrounding the Post smolt area level"){#fig-psnas10}
...
#### Assessment unit (or stock unit)
A stock unit for Atlantic salmon in the North Atlantic refers to a group of salmon that originates from a specific river or set of rivers, shares biological characteristics, and is treated as a distinct unit for assessment and management. This concept reflects both biological population structure and management boundaries, and is central to ICES and NASCO stock assessments.
It is used in the North Atlantic salmon model (Salmoglob) to separate different units of stock during the continental part of their life.
Assessment units are the term used by WGBAST to refer to a similar grouping of basins in the Baltic, it has been used here for consistency.
...
```{r}
#| label: refnas.tr_area_are inland assessment unit (stock unit) level
#| echo: FALSE
#| warning: FALSE
#| message: FALSE
...
![Map of the assessment unit (stock unit) level.](images/fig-au_inland_nas.png "A view with polygons surrounding the assessment units spread across Europe and North America"){#fig-auinnas1}
...
//| fig-cap: Hierarchical structure of WGEEL habitat database, dotted lines indicate that the element might not be used in some regions. Substocks and regional levels will only be finalised during model building and benchmarking, but lay out the ground for work.
...
<!-- ![Hierarchical structure of WGEEL habitat database.](images/fig-area_hierarchy_wgeel.png){#fig-area_hierarchy_eel) -->

This structure will be added into a table named `refeel.tr_area_are`. The global Stock is still the first layer and is made of a fusion between the Marine and the Inland Stock (@fig-wgeel_stock). It includes all catchments flowing into areas studied by WGEEL as well as all ICES Divisions and GFCM Subdivisions (for the Mediterranean).
...
#### Sub Component of the stock (Substock)
Sub components of the stock (substocks) are part of a whole stock but are assessed in separate models as they are geographically isolated during part of their lifecycle.
<!--Hilaire : Assessment is probably not the good wording here: remember wkfea and Alain: ICES only assesses the species at the stock level, so assessment unit is missleading. refer to sub-unit?-->
This query inserts a new spatial unit into the `refeel.tr_area_are` table, representing a **marine Sub Component** called **“Marine Barents”** as an example. It selects and merges (`ST_Union`) the geometries (`geom`) from the `ref.tr_fishingarea_fia` table, restricted to fishing areas whose division code (`fia_division`) matches one of the following values: `'27.14.a'`, `'27.5.a'`, `'27.2.b'`, `'27.2.a'`, `'27.1.b'`, `'27.1.a'`, or `'27.14.b'`.

The resulting unified geometry is converted to a multi-geometry (`ST_Multi`) and inserted as a single polygon into the `geom_polygon` column. A new area ID (`are_id`) is automatically generated using `nextval('refeel.seq')`, the parent area is set to `are_are_id = 2` (`Marine Stock`), the area code is labeled `'Marine Barents'`, and the level is defined as `'Substock'` via `are_lev_code`. The area is explicitly marked as marine (`are_ismarine = true`), and no line geometry is provided (`geom_line = NULL`).
 

```{sql}
#| eval: FALSE
#| echo: TRUE
#| code-summary: Code to create the Marine Substock level for WGEEL
#| warning: FALSE
#| message: FALSE
...
```{r}
#| label: refeel.tr_area_are Marine Substock level
#| echo: FALSE
#| warning: FALSE
#| message: FALSE
#| tbl-cap: Structure of the Marine Substock level in the referential table


fstock_lvl <- dbGetQuery(con_diaspara, 
  "SELECT are_id, are_are_id, are_code, are_lev_code, are_wkg_code, are_ismarine FROM refeel.tr_area_are
  WHERE are_id IN (1,2) OR are_lev_code = 'Substock' AND are_ismarine = true
  ORDER BY are_id;")
...
![Map of the Substock level for the WGEEL habitat database. The map is showing both inland and marine levels.](images/fig-wgeel_au.png){#fig-wgeel_au}


#### Regional
Regional units were discussed during WGEEL and might mostly be used during the building of regional models. They allow to split Substocks into smaller parts. At this stage, the final structure of the whole stock model is not defined, so this intermediate hierarchy will offer flexibility when building the model, and also when working on the regional models, which will require a finer spatial structure.
...
It starts with a Common Table Expression (`WITH region_selection`) that selects and merges (`ST_Union`) the geometries (`geom`) from the `ref.tr_fishingarea_fia` table at the **Subdivision** level (`fia_level = 'Subdivision'`). These fishing area geometries are spatially matched (`ST_Intersects`) with existing Substocks from the `refeel.tr_area_are` table. The selection is further restricted to subdivisions whose codes (`fia_subdivision`) belong to the specified list and to a single parent Substock identified by `are_id = 9`. The resulting geometries are grouped by the parent area and converted into a multi-geometry using `ST_Multi`.

The unified geometry produced by the CTE (common table expression) is then inserted as a single polygon into the `geom_polygon` column. A new area identifier (`are_id`) is automatically generated using `nextval('refeel.seq')`, while the parent area is set to the originating Substock (`are_are_id = are_id` from the CTE). The area code is defined as `'Western Mediterranean'`, the spatial level is set to `'Regional'` via `are_lev_code`, and the area is explicitly marked as marine (`are_ismarine = true`). No line geometry is provided (`geom_line = NULL`).
...
This query inserts a new spatial unit into the `refeel.tr_area_are` table, representing a **non-marine Substock** (@fig-wgeel_au) called **“Inland Barents”**.
...
#| eval: FALSE
#| echo: TRUE
#| code-summary: Code to create the Inland Substock level for WGEEL
#| warning: FALSE
#| message: FALSE
...
```{r}
#| label: refeel.tr_area_are Inland Substock level
#| echo: FALSE
#| warning: FALSE
#| message: FALSE
#| tbl-cap: Structure of the Inland Substock level in the referential table
...
It begins with a series of Common Table Expressions (CTEs) to progressively associate river segments with existing inland Substocks. The first CTE (`unit_riv`) identifies the main stretch of river (`main_riv`) whose first-order river segments (`ord_clas = 1`) spatially intersect (`ST_Intersects`) with non-marine Substocks (`are_lev_code = 'Assessment_unit'` and `are_ismarine = false`) in the `refeel.tr_area_are` table.

The second CTE (`all_segments`) retrieves all river segments belonging to these main rivers and associates them with the corresponding parent Substock. The third CTE (`catchments_with_riv`) links catchment polygons from `tempo.catchments_eel` to the river network by selecting catchments that spatially intersect the river segments, retaining their basin identifier (`main_bas`), catchment identifier (`hybas_id`), and geometry.

To avoid duplicates, the `deduplicated` CTE keeps a single record per catchment (`hybas_id`). The `merged` CTE then aggregates catchments by main basin (`main_bas`), assigns them to the corresponding parent Substock (`are_id`), and merges their geometries using `ST_Union`.

Finally, for each resulting river basin, a new area is inserted with a generated identifier (`are_id` via `nextval('refeel.seq')`), the parent area set to the associated Substock, and the basin code (`main_bas`) used as the area code. The spatial level is defined as `'River'`, the area is marked as non-marine (`are_ismarine = false`), and the merged geometry is stored as a multi-polygon (`ST_Multi`) in the `geom_polygon` column. No line geometry is provided (`geom_line = NULL`).
...
![Map of the River level for the WGEEL habitat database. Colours are related to the higher hierarchical level (Substock).](images/fig-wgeel_riv.png){#fig-wgeel_riv}
...
The EMU level is the spatial unit used in the eel regulation. It is nested within the country level. Some EMUs (for instance Andalusia) reports into different substocks (Mediterranean and Atlantic).
In the hierarchy, the substock hierarchical level is similar to the country (both have inland stock as a parent), but the hierarchical unit progress below substock to the river or lagoons while country is further divided into EMU but there is no lower hierarchy (@fig-area_hierarchy_bast)
...
[![DOI](images/zenodo.18155125.svg)](https://doi.org/10.5281/zenodo.18155125)
