...
Some developments were also done with the [EU-hydro](https://github.com/DIASPARAproject/WP3_habitat/blob/main/R/habitatdb_eu_hydro.qmd). This code relied on a lot of fixes made by Andrew French. These fixes could unfortunately be integrated by the time of finishing this report. However, that does not exclude the use of EU-Hydro (or other) data in the future. {>>Should we keep this into a final report?<<}
:::

::: {.callout-warning appearance="simple"}
The HydroSHEDS data, with a reach to the full project area, was used for creating the referential in this project. {>>I don't understand what you mean here<<}
...
from the area table, and some of the codes for basins will be based on HydroSHEDS.
{>>Should we keep this into the final report?<<}
:::
...
{>>The heading is a bit missleading: in a way in the previous step you have already built a database by converting geodatabases objects into tables or postgis objects. Would something like: "from a hydroshed spatial layers to a diadromous habitat db" be better?<<}
...
The most downstream points are gathered from `tempo.riveratlas_mds`, applying three filters: a country filter, an ICES ecoregion filter, and an exclusion condition.{>>I am not clear on what you mean by filter here<<}
...
#### Ecoregions
{>>the content of this section looks exactly the same as for the preivous section, Baltic, I think you are illustrating a larger process with the Baltic example, but you should clarify it. Moreover since Baltic an ecoregion by itself, the heading of the previous section might be a bit missleading<<}
We select distinct downstream points from `tempo.riveratlas_mds` using two spatial filters. The first selection includes points within 0.1 degrees of `ices_ecoregions.ices_ecoregions_20171207_erase_esri`, specifically for `objectid = 11`, and also within the larger buffer of 0.1 degrees of `tempo.ne_10m_admin_0_countries`, ensuring they are in Norway or Sweden.  {>>why do you need to ensure that they are in Norway or Sweden if we are workingg with ecoregion<<}
...
{>>you need a brief paragraph at the beginning to explain the strategy: that you are building specific schema for each group/ecoregion?<<}
...
{>>perhaps give a brief introduction on what is the problem here<<}
We construct the `tempo.oneendo_bisciber` {>>what is that?<<} table by generating a concave hull around the exterior boundary of the merged catchment shapes from `h_biscay_iberian.catchments`. This process creates a generalized polygon representing the area covered by these catchments. To improve spatial query performance, we added a **GIST** index on the geometry column.  

Next, we identify endorheic basins from `basinatlas.basinatlas_v10_lev12` that intersect with `tempo.oneendo_bisciber`. To ensure only relevant basins are selected, we exclude those already present in surrounding areas (`h_biscay_iberian.catchments`, `h_med_west.catchments`, `h_nsea_south.catchments`), and specific basins defined by `main_bas` values 2120017150, 2120017480, and 2120018070 that belong to another area. The remaining filtered basins are then inserted into `h_biscay_iberian.catchments`. {>>till now, you have only spoken about the Baltic so perhaps you should explain why you switch now to the Iberian peninsula<<}
...
Once the hydronetwork has been build and the link with ICES areas/ecoregions defined, the next step was to make the link with working group spatial units.
...
Several levels have been layed out (@fig-area_hierarchy_bast) depending on the needs of each working group.  {>>this figure is only about WGBAST, so prehaps we could avoid refering here to other working groups?<<}
...
Two subdivision groupings were used in the historical WGBAST database. They are dividing the Baltic into two main areas and are structured as followed : Gulf of Bothnia 300 (`27.3.d.30`,`27.3.d.31`) and Main Baltic 200 (`27.3.d.22`,`27.3.d.23`,`27.3.d.24`,`27.3.d.25`,`27.3.d.26`,`27.3.d.27`,`27.3.d.28`,`27.3.d.29`) {>>generally the d is not displayed in the notation, but I guess you keep it for consistency with ICES vocab<<}.
...
{>>we are delivering a database, so it might seem surprising to have optional table? either we deliver it or not?<<}
...
{>>is there a link between river catchments and countries?<<}
...
This level does not go lower than the country one but stays at the same level, assessment units stretching across several countries {>>I don't understand what you mean here<<}.
...
#### River
Within each assessment unit, rivers are split {>>split is missleading, it seems that is river is cut, while I think you rather mean that you classified rivers based on their outlet?<<} depending on their outlet. This is used to select specific rivers within each assessment unit with ease.
...
tables in Zenodo {>>since it is a final report and we are supposed to deliver finalised version, I would perhaps rephase and just say that some names are missing<<}
:::

#### River section
{>>put a definition of river section and what it is used for by WGBAST?<<}
...
{>>I disaggree with your picture and it does not seem consistent with the terminology used by WGNAS: a stock is the lowest spatial unit, their refer to stock complexes for grouped stock units. A stock is really a closed population from a biological point of view, reference points are defined at the stock level which is the stock unit.<<}

This structure is added into a table called `refnas.tr_area_are`.
As for **WGBAST** hierarchy, the first layer of the structure is the global Stock level (@fig-fullnas1). It includes all catchments that have a river flowing into areas studied by WGNAS {>>WGNAS also includes north-america, so we do not completely cover the picture<<}as well as all ICES Subareas corresponding to said areas of interest (@fig-stcknas1) {>>west greenland is not in? that is not an issue since a mixed fishery take place there?<<}. 

This query performs an update of the main **NEAC** area (North East Atlantic Commission), corresponding to the record with the identifier (`are_id`) equal to `1`. It begins by merging (`ST_Union`) all geometries found in the `geom_polygon` column of the `refnas.tr_area_are` table, then creates a concave hull around this union using the `ST_ConcaveHull` function. The resulting geometry is filtered to retain only shapes with an area (`ST_Area`) greater than `1`, thereby eliminating artifacts or small fragments. The final `UPDATE` statement modifies the record with `are_id = 1`: the parent area reference (`are_are_id`) is set to `NULL`, the area code becomes simply `'NEAC'`, the level remains `'Stock'`, the marine status is left undefined (`are_ismarine = NULL`), the main geometry is replaced with the filtered shape converted into a multipolygon via `ST_Multi`, and the linear geometry (`geom_line`) is set to `NULL`.
...
#### Subarea
{>>I am not compltely confortable with your classification since salmon from NEAC also migrate to west greenland, so within you NAC marine area<<}
...
#### Assessment unit
{>>not sure what assessment unit refer to. I suspect the wording is inapproppriate: assessment unit is the scale at which the assessment is done, so it corresponds to stock unit. Olmos had looked at the potential spatial distribution of post-smolts at sea arising from different stock units. If you are refering to these ones, then they are not assessment units. Post-smolt areas?<<}
After some discussions with WGNAS, it was decided to use as Assessment units at sea postsmolt areas from [Olmos et al. (2021)](https://doi.org/10.1111/gcb.14913). Those areas were recreated as close as possible using ICES Divisions.
...
{>>to be removed in the latest version<<
}

### Continental level
#### Stock
{>>same remark, this is not a stock<<}
...
{>>just a joke: EMU is within Country... I can't wait Isabel's reaction when we will speak about the Minho :-D <<}
...
#### Assessment unit
{>>Assessment is probably not the good wording here: remember wkfea and Alain: ICES only assesses the species at the stock level, so assessment unit is missleading. refer to sub-unit?<<}
...
#### Regional
{>>Where do the regions come from? to my knowledge we don't use them at all in WGEEL?<<}
...
#### Assessment_unit
{>>same as before, this is not the good wording for eel in my opinion<<}
...
#### Regional
{>>same as for the marine part, I don't understand where those regions come from<<}
...
#### EMU
{>>no!!!! unless you know that the Eel regulation will be revised in the future, and it is very unlikely, eel management plan will remain, so as EMU and ICES will keep receiving questions by EMU in WKEMP<<}
