...
Two subdivision groupings are in use in the WGBAST database to store some historical data. They are dividing the Baltic into two main areas and are structured as followed : Gulf of Bothnia 300 (`27.3.d.30`,`27.3.d.31`) and Main Baltic 200 (`27.3.d.22`,`27.3.d.23`,`27.3.d.24`,`27.3.d.25`,`27.3.d.26`,`27.3.d.27`,`27.3.d.28`,`27.3.d.29`).{>>Hilaire : generally the d is not displayed in the notation, but I guess you keep it for consistency with ICES vocab. Cédric Yes it's consistent with table content<<}
...
::: {.callout-caution}
Some names for the smallest rives are missing. Further improvement in the Vocabulary will be done by ICES.
:::
...
#### Sub Component
Sub component of the stock (substocks) are part of a whole stock but are assessed in separate models as they are geographically isolated during part of their lifecycle.
...
#### Regional
Regional units were discussed during WGEEL and might mostly be used during the building of regional models. They allow
to split Substocks into smaller parts. At this stage, the final structure of the whole stock model is not defined, so
this intermediate hierarchy will offer flexibility when building the model.
...
The unified geometry produced by the CTE is then inserted as a single polygon into the `geom_polygon` column. A new area identifier (`are_id`) is automatically generated using `nextval('refeel.seq')`, while the parent area is set to the originating Substock (`are_are_id = are_id` from the CTE). The area code is defined as `'Western Mediterranean'`, the spatial level is set to `'Regional'` via `are_lev_code`, and the area is explicitly marked as marine (`are_ismarine = true`). No line geometry is provided (`geom_line = NULL`).
...
The EMU level is the spatial unit used in the eel regulation. It is nested within the country level, but the hierarchy goes not beyond that level, as some EMUs (for instance Andalusia) reports into different substocks (Mediterranean and Atlantic).
